...
#define FILE_LOGNAME "fm_voucher_pol_device_set_brand.c"
 ...
 ******************************************************************************/
EXPORT_OP void
op_voucher_pol_device_set_brand(
...
static void
fm_voucher_pol_device_set_brand(
	pcm_context_t		*ctxp,
	pin_flist_t		*i_flistp,
	pin_errbuf_t		*ebufp);
...
/*******************************************************************************
 * Entry routine for the PCM_OP_VOUCHER_POL_DEVICE_SET_BRAND opcode
 ******************************************************************************/
void
op_voucher_pol_device_set_brand(
	cm_nap_connection_t	*connp,
	...
	pin_errbuf_t		*ebufp)
{
	 ...
	 * Insanity check.
	 */
	if (opcode != PCM_OP_VOUCHER_POL_DEVICE_SET_BRAND) {
		pin_set_err(ebufp, PIN_ERRLOC_FM,
			PIN_ERRCLASS_SYSTEM_DETERMINATE,
			PIN_ERR_BAD_OPCODE, 0, 0, opcode);
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
			"op_voucher_pol_device_set_brand opcode error", ebufp);
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
		"op_voucher_pol_device_set_brand input flist", i_flistp);
	...
	*o_flistpp = PIN_FLIST_COPY(i_flistp, ebufp);
	 ...
	 */
	fm_voucher_pol_device_set_brand(ctxp, i_flistp, ebufp);

	if (PIN_ERR_IS_ERR(ebufp)) {
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
			"op_voucher_pol_device_set_brand error", ebufp);
		PIN_FLIST_DESTROY_EX(o_flistpp, NULL);
		*o_flistpp = NULL;

	} else {
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
			"op_voucher_pol_device_set_brand return flist", *o_flistpp);
	}
 ...
 * fm_voucher_pol_device_set_brand()
 *
 * Inputs: flist with /device/voucher poid and account_obj
 * Outputs: void; ebuf set if errors encountered
 *
 * Description:
 * This function validates the input flist has a poid of type '/device/voucher',
	...
	pin_errbuf_t            *ebufp)
{
	...
	/*
	 * Validate the device is of type /device/voucher
	 * set an error code if the device type is not of 
	 * type voucher
	 */
	...
	if(vp != NULL){
		poid_type = (char *)PIN_POID_GET_TYPE((poid_t *)vp);
		if (strncmp(poid_type, PIN_OBJ_TYPE_DEVICE_VOUCHER
			, 15) != 0 ){

			PIN_ERR_LOG_MSG(PIN_ERR_LEVEL_ERROR,
			       "fm_voucher_pol_device_set_brand invalid poid type");
 ...
 *	for the /device/voucher
 * Outputs: void; ebuf set if errors encountered
 *
 * Description:
 * This function examines the account_obj value against the current account_obj
 * of the /device/voucher.  If they differ, the state of the device is checked to
 * enforce that only devices in the "NEW" state can have their brand 
 * changed.
 * 
	...
	pin_errbuf_t		*ebufp)
{
	...
	if (PIN_POID_COMPARE(ia_poidp, dba_poidp, 0, ebufp) != 0) {
		...
		if(vp != NULL){
			...
			if(state_id != PIN_VOUCHER_NEW_STATE)
			{
				/*
				 * validation fails 
				 */
