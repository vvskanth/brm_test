...
#ifndef lint
static	char	Sccs_id[] = "@(#)$Id: fm_subscription_pol_provisioning.c /cgbubrm_7.5.0.portalbase/1 2023/05/30 06:17:29 sreejs Exp $";
#endif
...
tag_fn_telephony(
        ...
        while ((a_flistp = PIN_FLIST_ELEM_GET_NEXT(c_flistp,
                ...
                if (ctag_name[0] == '\0' || PIN_ERR_IS_ERR(ebufp)) {
	...
	)
{
	...
	obj_typep = (char *)PIN_POID_GET_TYPE(pp);
	pin_memset(msg, sizeof(msg), '\0', sizeof(msg));
	...
	if (c_flistp == (pin_flist_t *)NULL){
		...
		pin_snprintf(msg, sizeof(msg), "No config object found for object : %s", obj_type_str);
	...
	pin_errbuf_t	*ebufp)
{
	 ...
	 ***********************************************************/
	pin_memset(s_template, sizeof(s_template), 0, sizeof(s_template));
	pin_snprintf(s_template, sizeof(s_template), "%s", (char *)"select X from " \
	...
	PIN_FLIST_FLD_PUT(tmp_flistp, PIN_FLD_POID, arg_pdp, ebufp);
