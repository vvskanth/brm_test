...
#ifndef lint
static  char    Sccs_id[] = "@(#)%Portal Version: fm_price_pol_valid_transition.c:ServerIDCVelocityInt:1:2006-Sep-06 16:36:55 %";
#endif
...
#define FILE_SOURCE_ID  "fm_price_pol_valid_transition.c (1.7)"
 
/*******************************************************************
 * Main routine for the PCM_OP_PRICE_POL_VALID_TRANSITION operation.
 *******************************************************************/
EXPORT_OP void
op_price_pol_valid_transition(
        ...
        pin_errbuf_t		*ebufp)
{
	...
	*r_flistpp = NULL;
	 ...
	 * Insanity check.
	 */
	if (opcode != PCM_OP_PRICE_POL_VALID_TRANSITION) {
		pin_set_err(ebufp, PIN_ERRLOC_FM,
			PIN_ERRCLASS_SYSTEM_DETERMINATE,
			PIN_ERR_BAD_OPCODE, 0, 0, opcode);
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
			"bad opcode in op_price_pol_valid_transition", ebufp);
		return;
	}
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
		"op_price_pol_valid_transition input flist", i_flistp);
	...
	*r_flistpp = PIN_FLIST_CREATE(ebufp);
	...
	if (PIN_ERR_IS_ERR(ebufp)) {
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_DEBUG,
			"op_price_pol_valid_transition error", ebufp);
	} else {
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
			"op_price_pol_valid_transition return flist", *r_flistpp);
	}
