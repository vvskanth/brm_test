...
/*******************************************************************
 * Contains the PCM_OP_ORDER_POL_CREATE operation.
 *******************************************************************/
...
#define FILE_LOGNAME "fm_order_pol_create.c(1)"
	...
	pin_errbuf_t		*ebufp)
{
	...
	*r_flistpp = NULL;
	 ...
	 * Insanity check.
	 ***********************************************************/
	if (opcode != PCM_OP_ORDER_POL_CREATE) {
		pin_set_err(ebufp, PIN_ERRLOC_FM,
			PIN_ERRCLASS_SYSTEM_DETERMINATE,
			PIN_ERR_BAD_OPCODE, 0, 0, opcode);
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
			"op_order_pol_create opcode error", ebufp);
	...
	PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
		"op_order_pol_create input flist", i_flistp);

	/***********************************************************
	 * Main rountine for this opcode
	 ***********************************************************/
	fm_order_pol_create(ctxp, flags, i_flistp, r_flistpp, ebufp);
	...
	if (PIN_ERR_IS_ERR(ebufp)) {
		...
		PIN_ERR_LOG_EBUF(PIN_ERR_LEVEL_ERROR,
			"op_order_pol_create error", ebufp);
	} else {
		PIN_ERR_LOG_FLIST(PIN_ERR_LEVEL_DEBUG,
			"op_order_pol_create output flist", *r_flistpp);
	}
	...
	pin_errbuf_t            *ebufp)
{
	...
	PIN_ERR_CLEAR_ERR(ebufp);
        ...
        if (strcmp(poid_type, PIN_OBJ_TYPE_ORDER_VOUCHER) == 0) {
                 ...
                 */
                PCM_OP(ctxp, PCM_OP_VOUCHER_POL_ORDER_CREATE, flags, i_flistp,
                        r_flistpp, ebufp);
        }
        else
        {
