...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:java="http://www.oracle.com/XSL/Transform/java/" xmlns:pxslt="http://www.oracle.com/XSL/Transform/java/com.portal.pxslt.PXSLTUtils" version="1.0" exclude-result-prefixes="java pxslt">
<xsl:output method="html" indent = "yes"/>
    ...
    <TD align="left">
      <TABLE width="100%" border="1" cellspacing="0">
		...
		<xsl:variable name="date1"> 
			<xsl:value-of select="/invoice/BILLINFO/END_T" />
		</xsl:variable>
		<!--
		Timezone and format confirms to Java's SimpleDateFormat specification 
		for Ex:  "EEE, d MMM yyyy HH:mm:ss Z"	
		and result is Wed, 4 Jul 2001 12:08:56 -0700
		-->
		<xsl:variable name="fmt_date1" select="pxslt:getFormattedDate($date1,'EEE, d MMM yyyy HH:mm:ss Z')"/>
		<xsl:value-of select="$fmt_date1"/>
		 </FONT>
          </TD>
          <TD align="center">
		...
		<xsl:variable name="date2"> 
			<xsl:value-of select="/invoice/BILLINFO/DUE_T" />
		</xsl:variable>
		<xsl:variable name="fmt_date2" select="pxslt:getFormattedDate($date2,'MMM dd yyyy')"/>
		<xsl:value-of select="$fmt_date2"/>
		 </FONT>
          </TD>
          <TD align="center">
		<FONT size="2"> 
  ...
  <xsl:template name="promotion-template">
    ...
    <xsl:for-each select="$promotion-list">
		...
		<xsl:variable name="fmt_date2" select="pxslt:getFormattedDateWithTimeZone($date2,'MMM dd yyyy','Indian/Maldives')"/>
		<xsl:value-of select="$fmt_date2"/>
                 </FONT>
          </TD>
          <TD align="center">
		...
		<xsl:variable name="fmt_date3" select="pxslt:getFormattedDate($date3,'MMM dd yyyy')"/>
		<xsl:value-of select="$fmt_date3"/>
                </xsl:otherwise>
                </xsl:choose>
                 </FONT>
          </TD>
    ...
    <xsl:sort select="ITEM_NO"/>
      <xsl:for-each select="EVENTS">
		...
		<xsl:variable name="date4"> 
			<xsl:value-of select="END_T" />
		</xsl:variable>
		<xsl:variable name="fmt_date4" select="pxslt:getFormattedDate($date4,'MMM dd yyyy')"/>
		<xsl:value-of select="$fmt_date4"/>
	      </FONT></TD>
